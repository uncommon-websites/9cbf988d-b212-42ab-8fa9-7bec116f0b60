...
<SectionWrapper withBorderBottom={true} withBorderTop={true} className="py-6">
  <nav class="flex items-center justify-between px-4 md:px-12">
    ...
    <div class="flex items-center space-x-3">
      ...
      <span class="font-semibold text-xl tracking-tight">Cashfeed</span>
    </div>
    ...
    <!-- Action -->
    <div class="flex items-center space-x-3">
      <a href="#" class="hidden md:inline-block px-6 py-2 text-gray-600 hover:text-gray-900 text-sm font-medium transition-colors">
        Login
      </a>
      <a href="#" class="inline-block px-6 py-2 bg-black hover:bg-gray-800 text-white text-sm font-medium rounded transition-colors">
        Get Started
      </a>
    </div>
  </nav>
